...
// Ensure process.env for web/SSR
...
export const useAuthStore = create<AuthState>((set) => ({
  ...
  initializeAuth: async () => {
    ...
    try {
      ...
      console.log('🔧 initializeAuth: Got auth_token:', token ? 'EXISTS' : 'NULL');
      const refreshToken = await getItem("refresh_token");
      console.log('🔧 initializeAuth: Got refresh_token:', refreshToken ? 'EXISTS' : 'NULL');
      const approved = await getItem("login_approved");
      console.log('🔧 initializeAuth: Got login_approved:', approved);
      ...
      if (token && approved === 'true' && !requireOtpOnStart) {
        ...
        try {
          ...
          console.log('🔧 initializeAuth: Profile fetch complete:', profileResp.success ? 'SUCCESS' : 'FAILED');
          ...
          if (profileResp.success && profileResp.data) {
            ...
            console.log('🔧 initializeAuth: Profile fetch failed:', unauthorized ? 'UNAUTHORIZED' : 'OTHER_ERROR');
