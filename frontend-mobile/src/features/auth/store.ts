...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  initializeAuth: async () => {
    ...
    try {
      ...
      // Get stored tokens
      console.log('🔧 initializeAuth: Getting stored tokens... (start Promise.all getItem)');
      ...
      if (token && approved === 'true' && !requireOtpOnStart) {
        ...
        try {
          ...
          console.log('🔧 initializeAuth: Profile fetch complete:', 
            profileResp && profileResp.success ? 'SUCCESS' : 'FAILED', { profileResp });
          ...
          if (profileResp.success && profileResp.data) {
            ...
            // Fetch capabilities separately (ensure capabilities is always an array)
            ...
            if (unauthorized) {
              ...
              console.log('🔧 initializeAuth: Keeping session for retry (network error)');
              // Do not mark user as authenticated when profile fetch failed due to non-auth error.
              set({
                user: null,
                token,
                refreshToken,
          ...
          // Network or other error: keep token but do not mark authenticated
          // Don't show error message for network issues during initialization
          set({
            user: null,
            token,
            refreshToken,
