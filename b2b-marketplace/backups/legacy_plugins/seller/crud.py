...
# Seller CRUD
# -----------------------------
async def create_seller(db: AsyncSession, seller_data: SellerCreate, user_id: int, new_user_id: str = None) -> Seller:
...
async def get_seller(db: AsyncSession, seller_id: int, new_seller_id: str = None) -> Seller | None:
    return await db.get(Seller, seller_id)

async def get_seller_by_user_id(db: AsyncSession, user_id: int, new_user_id: str = None) -> Seller | None:
    result = await db.execute(select(Seller).where(Seller.new_user_id == new_user_id if new_user_id else Seller.user_id == user_id))
    return result.scalars().first()

async def update_seller(db: AsyncSession, seller_id: int, user_id: int, new_user_id: str = None: int) -> Seller | None:
    seller = await db.get(Seller, seller_id)
    if seller and seller.new_user_id == new_user_id if new_user_id else seller.user_id == user_id:
...
async def delete_seller(db: AsyncSession, seller_id: int, user_id: int, new_user_id: str = None: int) -> bool:
    seller = await db.get(Seller, seller_id)
    if seller and seller.new_user_id == new_user_id if new_user_id else seller.user_id == user_id:
...
async def get_store_analytics(db: AsyncSession, seller_id: int, days: int = 30) -> StoreAnalytics:
    ...
    total_products_result = await db.execute(
        ...
        .where(Product.new_seller_id == new_seller_id if new_seller_id else Product.seller_id == seller_id)
    ...
    orders_result = await db.execute(
        ...
        .where(Order.new_seller_id == new_seller_id if new_seller_id else Order.new_seller_id == new_seller_id if new_seller_id else Order.seller_id == seller_id)
...
# Seller Statistics
# -----------------------------
async def get_seller_statistics(db: AsyncSession, seller_id: int, new_seller_id: str = None) -> Dict[str, Any]:
