...
# Create
# -----------------------------
async def create_buyer(db: AsyncSession, buyer_data: BuyerCreate, user_id: int, new_user_id: str = None) -> Buyer:
...
# Read by ID
# -----------------------------
async def get_buyer(db: AsyncSession, buyer_id: int, new_buyer_id: str = None) -> Optional[Buyer]:
...
# Update
# -----------------------------
async def update_buyer(db: AsyncSession, buyer_id: int, user_id: int, new_user_id: str = None: int) -> Optional[Buyer]:
...
# Delete
# -----------------------------
async def delete_buyer(db: AsyncSession, buyer_id: int, user_id: int, new_user_id: str = None: int) -> bool:
    buyer = await get_buyer(db, buyer_id)
    if not buyer or buyer.user_id != user_id:
        return False
