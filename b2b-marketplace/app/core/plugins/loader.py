...
class PluginLoader:
    ...
    def discover(self) -> Dict[str, str]:
        ...
        if fs_path and os.path.isdir(fs_path):
            ...
            # Common dev roots
            candidate_roots.extend(["/workspace", "/code", "/b2b-marketplace"])  # tolerate container paths
            # Directory relative to this file (../../.. -> repo root)
            here = os.path.dirname(__file__)
            repo_root = os.path.abspath(os.path.join(here, "..", "..", ".."))
            candidate_roots.append(repo_root)
            # Also try the parent directory of the current working directory
        ...
        print(f"[discover] Looking for plugins in: {pkg_path}")
        for _, name, ispkg in pkgutil.iter_modules([pkg_path]):
            print(f"[discover] Found module: {name}, is_package: {ispkg}")
            if name in excluded_plugins:
                print(f"[discover] Skipping excluded plugin: {name}")
                continue  # Skip legacy plugins
            if not ispkg:
                print(f"[discover] Skipping non-package: {name}")
                continue
            module_name = f"{package}.{name}"
            try:
                print(f"[discover] Importing module: {module_name}")
                mod = importlib.import_module(module_name)
                plugin_class: Type[PluginBase] | None = getattr(mod, "Plugin", None)
                if not plugin_class:
                    print(f"[discover] No Plugin class found in {module_name}")
                    continue
                # Instantiate once to read slug
                instance = plugin_class()
                slug = instance.slug or name
                print(f"[discover] Found plugin: {slug}")
                discovered[slug] = module_name
            except Exception as e:
                print(f"[discover] Failed to import {module_name}: {e}")
        print(f"[discover] Total plugins discovered: {len(discovered)}")
    ...
    async def load_all(self, app: FastAPI, engine: AsyncEngine) -> List[PluginBase]:
        ...
        for slug, module_name in order:
            try:
                ...
                print(f"[loader] Plugin {slug} routes registered")
            except Exception as e:
                print(f"[loader] Failed to load '{slug}' from {module_name}: {e}")
                import traceback
                print(f"[loader] Traceback: {traceback.format_exc()}")
