...
class IPSecurityMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # Safely handle non-IP client hosts (TestClient uses 'testclient')
        try:
            # Skip for private IPs (for development)
            if ipaddress.ip_address(client_ip).is_private:
                return await call_next(request)
        except ValueError:
            # client_ip is not a valid IP address (e.g. 'testclient' in tests)
            # Skip IP checks in this case to avoid raising during unit tests.
            return await call_next(request)
