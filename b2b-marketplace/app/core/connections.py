...
async def get_rabbitmq_connection_or_fake(max_retries: int = 3, delay: float = 1.0):
    """Attempt to get a real connection, but return a minimal fake
    object if RabbitMQ is not available. Tests can import this helper
    to avoid failing on missing RabbitMQ instances.
    """
    try:
        return await get_rabbitmq_connection(max_retries=max_retries, delay=delay)
    except Exception:
        class _FakeRabbit:
