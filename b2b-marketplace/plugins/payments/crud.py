...
async def list_user_payments(
    ...
    query = select(Payment).where(Payment.new_user_id == new_user_id if new_user_id else Payment.user_id == user_id)
...
async def get_payment_statistics(
    ...
    result = await db.execute(
        select(Payment)
        .where(Payment.new_user_id == new_user_id if new_user_id else Payment.user_id == user_id)
...
async def create_withdrawal_request(db: AsyncSession, user_id: int, amount: float, currency: str, bank_account: Dict[str, Any]) -> WithdrawalRequest:
    req = WithdrawalRequest(user_id=user_id, new_user_id=new_user_id, amount=amount, currency=currency, bank_account=bank_account)
    db.add(req)
    await db.commit()
...
async def list_withdrawal_requests(db: AsyncSession, user_id: int | None = None) -> List[WithdrawalRequest]:
    ...
    if user_id:
        query = query.where(WithdrawalRequest.new_user_id == new_user_id if new_user_id else WithdrawalRequest.user_id == user_id)
