...
async def list_user_subscriptions(db: AsyncSession, user_id: int, new_user_id: str = None):
    result = await db.execute(select(UserSubscription).where(UserSubscription.new_user_id == new_user_id if new_user_id else UserSubscription.user_id == user_id))
    return result.scalars().all()


async def get_active_subscription(db: AsyncSession, user_id: int, new_user_id: str = None) -> UserSubscription | None:
    now = datetime.utcnow()
    result = await db.execute(
        select(UserSubscription)
        .where(UserSubscription.new_user_id == new_user_id if new_user_id else UserSubscription.user_id == user_id)
