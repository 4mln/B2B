...
async def create_rfq_endpoint(
    ...
    return await crud.create_rfq(db, buyer_id=user.id, new_buyer_id=user.id if hasattr(user, 'id') and str(user.id).startswith('USR-') else None, data=payload)
...
async def create_quote_endpoint(
    ...
    return await crud.create_quote(db, seller_id=user.id, new_seller_id=user.id if hasattr(user, 'id') and str(user.id).startswith('USR-') else None, data=payload)
