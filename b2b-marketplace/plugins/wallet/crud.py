...
async def get_user_wallets(db: AsyncSession, user_id: int, new_user_id: str = None) -> List[Wallet]:
...
async def get_user_wallet_by_currency(db: AsyncSession, user_id: int, currency: str, new_user_id: Optional[str] = None) -> Optional[Wallet]:
    result = await db.execute(
        select(Wallet).where(
            and_(
                Wallet.new_user_id == new_user_id if new_user_id else Wallet.user_id == user_id,
