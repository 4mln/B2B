...
# Chat Room CRUD
async def create_chat_room(db: Session, chat_data: ChatRoomCreate, creator_id: int, new_user_id: Optional[str] = None) -> ChatRoom:
...
async def get_chat_room(db: Session, chat_room_id: int, user_id: int, new_user_id: Optional[str] = None) -> Optional[ChatRoom]:
    """Get chat room if user is a participant"""
    return db.query(ChatRoom).join(ChatParticipant).filter(
        and_(
            ChatRoom.id == chat_room_id,
            ChatParticipant.new_user_id == new_user_id if new_user_id else ChatParticipant.user_id == user_id,
            ChatParticipant.left_at.is_(None)
        )
    ).first()


async def get_user_chats(db: Session, user_id: int, skip: int = 0, limit: int = 50, new_user_id: Optional[str] = None) -> List[ChatRoom]:
...
async def get_messages(db: Session, chat_room_id: int, user_id: int, new_user_id: Optional[str] = None, 
...
async def delete_message(db: Session, message_id: int, user_id: int, new_user_id: Optional[str] = None) -> bool:
...
async def mark_message_read(db: Session, message_id: int, user_id: int, new_user_id: Optional[str] = None) -> bool:
    """Mark a message as read"""
    # Check if already marked as read
    existing = db.query(MessageReadStatus).filter(
        and_(
            MessageReadStatus.message_id == message_id,
            MessageReadStatus.new_user_id == new_user_id if new_user_id else MessageReadStatus.user_id == user_id
        )
    ).first()
...
async def mark_chat_read(db: Session, chat_room_id: int, user_id: int, new_user_id: Optional[str] = None) -> bool:
...
# Participant CRUD
async def add_participant(db: Session, chat_room_id: int, user_id: int, new_user_id: Optional[str] = None, 
    ...
    participant = ChatParticipant(
        ...
        user_id=user_id, new_user_id=new_user_id,
        is_admin=is_admin
    )
    db.add(participant)
    db.commit()
...
async def remove_participant(db: Session, chat_room_id: int, user_id: int, new_user_id: Optional[str] = None) -> bool:
...
async def create_invitation(db: Session, chat_room_id: int, 
                           invited_by: int, user_id: int, new_user_id: Optional[str] = None) -> ChatInvitation:
...
# Utility functions
async def get_unread_count(db: Session, chat_room_id: int, user_id: int, new_user_id: Optional[str] = None) -> int:
