...
@router.get("/rooms/{chat_room_id}", response_model=schemas.ChatRoomWithParticipants)
async def get_chat_room(
    chat_room_id: int,
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(lambda: __import__("importlib").import_module("app.db.session").get_session)
...
@router.put("/rooms/{chat_room_id}", response_model=schemas.ChatRoomWithParticipants)
async def update_chat_room(
    chat_room_id: int,
    ...
    updated_chat = await crud.update_chat_room(db, chat_room_id, chat_data.dict(exclude_unset=True))
    if not updated_chat:
        raise HTTPException(status_code=404, detail="Chat room not found")
...
@router.delete("/rooms/{chat_room_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_chat_room(
    ...
    """Delete a chat room"""
    success = await crud.delete_chat_room(db, chat_room_id, current_user.id)
    if not success:
        raise HTTPException(status_code=404, detail="Chat room not found")
...
@router.post("/rooms/{chat_room_id}/participants", response_model=schemas.ChatParticipantResponse)
async def add_participant_to_room(
    ...
    new_participant = await crud.add_participant(
        db, chat_room_id, participant_data.user_id, None, participant_data.is_admin
    )
    
    return {"message": "Participant added successfully", "participant_id": new_participant.id}


@router.delete("/rooms/{chat_room_id}/participants/{user_id}", status_code=status.HTTP_204_NO_CONTENT)
...
async def get_chat_messages(
    ...
    messages = await crud.get_messages(db, chat_room_id, current_user.id, None, skip, limit)
...
async def mark_message_as_read(
    ...
    await crud.mark_message_read(db, message_id, current_user.id)
    return {"message": "Message marked as read"}


@router.post("/rooms/{chat_room_id}/read", status_code=status.HTTP_204_NO_CONTENT)
