...
from plugins.analytics.models import AnalyticsEvent

async def create_event(db: AsyncSession, event_type: str, event_data: dict, user_id: int | None = None, new_user_id: str | None = None):
    ev = AnalyticsEvent(event_type=event_type, event_data=event_data, user_id=user_id, new_user_id=new_user_id)
    db.add(ev)
    await db.commit()
    await db.refresh(ev)
    return ev
...
async def list_events(db: AsyncSession, limit: int = 100):
    result = await db.execute(AnalyticsEvent.__table__.select().limit(limit))
...
def get_analytics_events(
    ...
    if user_id:
        query = query.filter(AnalyticsEvent.new_user_id == new_user_id if new_user_id else AnalyticsEvent.user_id == user_id)
    if entity_type:
...
def get_user_analytics(
    ...
    query = db.query(UserAnalytics).filter(
        UserAnalytics.new_user_id == new_user_id if new_user_id else UserAnalytics.user_id == user_id,
...
def get_seller_analytics(
    ...
    query = db.query(SellerAnalytics).filter(
        SellerAnalytics.new_seller_id == new_seller_id if new_seller_id else SellerAnalytics.seller_id == seller_id,
...
def get_user_dashboards(
    ...
    """Get user dashboards"""
    query = db.query(Dashboard).filter(Dashboard.new_user_id == new_user_id if new_user_id else Dashboard.user_id == user_id)
...
def get_user_exports(
    ...
    query = db.query(DataExport).filter(DataExport.new_user_id == new_user_id if new_user_id else DataExport.user_id == user_id)
