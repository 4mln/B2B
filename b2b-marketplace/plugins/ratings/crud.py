...
async def list_ratings_for_user(db: AsyncSession, user_id: int, new_user_id: str = None) -> list[Rating]:
    result = await db.execute(select(Rating).where(Rating.ratee_id == user_id))
    return result.scalars().all()


async def get_reputation_score(db: AsyncSession, user_id: int, new_user_id: str = None) -> float:
