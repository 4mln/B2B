...
async def get_user_badges(db: AsyncSession, user_id: int, new_user_id: str = None) -> List[BadgeOut]:
    result = await db.execute(
        select(UserBadge)
        .where(UserBadge.new_user_id == new_user_id if new_user_id else UserBadge.user_id == user_id)
...
async def get_gamification_progress(db: AsyncSession, user_id: int, new_user_id: str = None) -> GamificationProgress:
...
async def award_badge_to_user(db: AsyncSession, user_id: int, badge_id: int) -> BadgeOut:
    ...
    existing = await db.execute(
        select(UserBadge)
        .where(UserBadge.new_user_id == new_user_id if new_user_id else UserBadge.user_id == user_id, UserBadge.badge_id == badge_id)
    )
