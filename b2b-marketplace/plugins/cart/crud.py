...
# Get cart by user
async def get_cart_by_user(db: AsyncSession, user_id: int, new_user_id: str = None) -> Cart | None:
    result = await db.execute(select(Cart).where(Cart.new_user_id == new_user_id if new_user_id else Cart.user_id == user_id, Cart.is_active == True))
