...
def get_user_notifications(
    ...
    query = db.query(Notification).filter(Notification.new_user_id == new_user_id if new_user_id else Notification.user_id == user_id)
    ...
    unread_count = db.query(Notification).filter(
        Notification.new_user_id == new_user_id if new_user_id else Notification.user_id == user_id,
...
def mark_notification_read(db: Session, notification_id: int, user_id: int, new_user_id: str | None = None) -> Optional[NotificationOut]:
...
def mark_notifications_read(db: Session, notification_ids: List[int], user_id: int, new_user_id: str = None) -> NotificationMarkReadResponse:
...
def mark_all_notifications_read(db: Session, user_id: int, new_user_id: str = None) -> int:
...
def send_bulk_notifications(db: Session, bulk_request: BulkNotificationRequest) -> List[NotificationSendResponse]:
    ...
    for user_id in bulk_request.user_ids:
        send_request = NotificationSendRequest(
            user_id=user_id, new_user_id=None,
...
def get_user_notification_preferences(db: Session, user_id: int, new_user_id: str = None) -> List[UserNotificationPreferenceOut]:
    """Get all notification preferences for a user"""
    preferences = db.query(UserNotificationPreference).filter(
        UserNotificationPreference.new_user_id == new_user_id if new_user_id else UserNotificationPreference.user_id == user_id
    ).all()
...
def delete_user_notification_preference(db: Session, user_id: int, notification_type: NotificationType) -> bool:
    ...
    # new_user_id optional not passed from caller in many places; use user_id only
    preference = db.query(UserNotificationPreference).filter(
...
def get_user_notification_subscriptions(db: Session, user_id: int, new_user_id: str = None) -> List[NotificationSubscriptionOut]:
    """Get all notification subscriptions for a user"""
    subscriptions = db.query(NotificationSubscription).filter(
        NotificationSubscription.new_user_id == new_user_id if new_user_id else NotificationSubscription.user_id == user_id,
