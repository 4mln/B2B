...
def get_admin_user_by_user_id(db: Session, user_id: int, new_user_id: str = None) -> Optional[AdminUser]:
    """Get admin user by user ID"""
    return db.query(AdminUser).filter(AdminUser.new_user_id == new_user_id if new_user_id else AdminUser.user_id == user_id).first()
...
def create_support_ticket(db: Session, ticket_data: SupportTicketCreate, user_id: int, new_user_id: str = None) -> SupportTicket:
